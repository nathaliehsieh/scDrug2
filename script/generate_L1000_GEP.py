...
if __name__ == '__main__':
    ...
    if do_detect:
        ...
        average_gep = pd.read_csv(args.gep, delimiter='\t', indexl_col=0).mean(axis=1).to_numpy()
        # normalize to TPM
        average_gep = average_gep / average_gep.sum() * 1e6
        ...
        for c in cellline_gep.columns:
            if(p:=(pearsonr(average_gep[mutual_genes], cellline_gep.loc[mutual_genes, c].to_numpy())) > max_p):
                max_p = p
                cell = c
